...
def create_table_portfolio(cursor):
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS portfolio_record (id INTEGER PRIMARY KEY AUTOINCREMENT, first_name TEXT, last_name TEXT, email TEXT, phone TEXT, address TEXT, description TEXT, image TEXT)"
    )

def create_table_experience(cursor):
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS experience_record (id INTEGER PRIMARY KEY AUTOINCREMENT, id_user INTEGER FOREIGN KEY, starting_date TEXT, ending_date TEXT, company TEXT, role TEXT)"
    )
 
def create_table_education(cursor):
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS education_record (id INTEGER PRIMARY KEY AUTOINCREMENT, id_user INTEGER FOREIGN KEY, starting_date TEXT, ending_date TEXT, institution TEXT, specialization TEXT)"
    )